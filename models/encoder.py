...
class Encode2Project(nn.Module):
    def __init__(self, hparams):
        super().__init__()
        ...
        self.projection_simclr = nn.Sequential(OrderedDict(projection_layers_simclr))

        projection_layers_augclr = [
            ('fc1', nn.Linear(self.encoder_dim, self.encoder_dim, bias=False)),
            ('bn1', nn.BatchNorm1d(self.encoder_dim)),
            ('relu1', nn.ReLU()),
            ('fc2', nn.Linear(self.encoder_dim, 128, bias=False)),
        ...
        self.projection_augclr = nn.Sequential(OrderedDict(projection_layers_augclr))

    def forward(self, x, out='z'):
        h = self.convnet(x)
        if out == 'h':
            return h
        return self.projection_simclr(h), self.projection_augclr(h)
